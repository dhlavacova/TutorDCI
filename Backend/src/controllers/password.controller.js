...
export const changePassword = async (req, res) => {
    ...
    try {
        if (!req.user) {
            ...
            return res.status(400).json({message: ["Misstake autentikacion."]});
        }
        ...
        if (!passwordMatch) {
            ...
            const userUpdatedPassword = await User.findOneAndUpdate({_id: userId}, {
                $set: {
    ...
    try {
        if (!req.body) {
            ...
            if (!isUserinRegister) {
                ...
                const userUpdatedPassword = await User.findOneAndUpdate({email: email}, {
                    $set: {
                        password: passwordHash,
